...
$a_u$: That is, writing $\mathbb{P}_{x_p}[\cdot]_$ to indicate the probability that an event is true when sampled over all positions and 
images, we define the threshold $t_u \equiv max_t\mathbb{P}_{x_p}[a_u(x , p) > t]\geq 0.01_$.
Activation regions above the threshold are highlighted in visualizations by
